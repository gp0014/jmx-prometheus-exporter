...
public class JmxCollector extends Collector implements Collector.Describable {
    ...
    private static class Config {
        ...
        List<JMXConfig> jmxConfigs = new ArrayList<>();
    ...
    private Config loadConfig(Map<String, Object> yamlConfig) throws MalformedObjectNameException {
        ...
        final List<Map<String, Object>> jmxConfigs = (List<Map<String, Object>>) yamlConfig.get("jmxConfigs");
        for (final Map<String, Object> jmxConfig : jmxConfigs) {
            JMXConfig jmxConf = new JMXConfig();
            if (jmxConfig.containsKey("hostPort")) {
                if (jmxConfig.containsKey("jmxUrl")) {
                    throw new IllegalArgumentException("At most one of hostPort and jmxUrl must be provided");
                }
                jmxConf.jmxUrl = "service:jmx:rmi:///jndi/rmi://" + jmxConfig.get("hostPort") + "/jmxrmi";
            } else if (jmxConfig.containsKey("jmxUrl")) {
                jmxConf.jmxUrl = (String) jmxConfig.get("jmxUrl");
            }

            if (jmxConfig.containsKey("username")) {
                jmxConf.username = (String) jmxConfig.get("username");
            }

            if (jmxConfig.containsKey("password")) {
                jmxConf.password = (String) jmxConfig.get("password");
            }

            if (jmxConfig.containsKey("ssl")) {
                jmxConf.ssl = (Boolean) jmxConfig.get("ssl");
            }
            cfg.jmxConfigs.add(jmxConf);
        }
    ...
    public List<MetricFamilySamples> collect() {
        ...
        final Config config = getLatestConfig();

        MatchedRulesCache.StalenessTracker stalenessTracker = new MatchedRulesCache.StalenessTracker();
        final Receiver receiver = new Receiver(config, stalenessTracker);
        ...
        for (final JMXConfig jmxConfig : config.jmxConfigs) {
            executorService.submit(() -> {
                JmxScraper scraper = new JmxScraper(jmxConfig.jmxUrl, jmxConfig.username, jmxConfig.password, jmxConfig.ssl,
        ...
        samples.add(new MetricFamilySamples.Sample(
                "jmx_scrape_error", new ArrayList<>(), new ArrayList<>(), error.doubleValue()));
